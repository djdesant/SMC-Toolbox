...
% Linear Motor with Disturbance compensation
...
for k = 1:length(u)-1
    ...
    if k == 1
        ...
        Fh(k) = 1/h*(e2(k)-e2(k-1)) + b*u(k-1) + a*e2(k-1) - ...
            (a*xr_dot(k-1)+xr_ddot(k-1));
    end
    
    % control signal
    u(k) = 1/(h*b)*((1+c1*h-h*a)*e2(k) + c1*e1(k) + h*Fh(k) + ...
        h*(a*xr_dot(k)+xr_ddot(k)) + c2*sig(e1(k)+h*e2(k),alpha));
